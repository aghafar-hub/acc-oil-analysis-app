...
export function rowToAction(row) {
  const [acNo, equipmentCode, description, oilType, revisionDate, sampleDate,
    sampleResult, sampleAnalysis, lastChange, status, contractorAction,
    contractor, completedDate, prevMonthAgreedAction, accAction, agreedAction] = row;
  return {
    acNo, equipmentCode, unitId: equipmentCode, description, oilType,
    revisionDate: formatDate(revisionDate), sampleDate: formatDate(sampleDate),
    sampleResult, sampleAnalysis, lastChange: formatDate(lastChange), status,
    contractorAction, contractor, completedDate: formatDate(completedDate),
...
export function actionToRow(a) {
  return [
    a.acNo || "", a.equipmentCode || a.unitId || "", a.description || "",
    a.oilType || "", a.revisionDate || "", a.sampleDate || "",
    a.sampleResult || "", a.sampleAnalysis || "", a.lastChange || "",
    a.status || "", a.contractorAction || "", a.contractor || "",
    a.completedDate || "", a.prevMonthAgreedAction || "", a.accAction || "",
    a.agreedAction || "",
  ];
}
...
export function rowToOilChange(row) {
  ...
  const oilType = row[4];
  return {
    equipmentCode, assetName: equipmentCode, lubricationPoint,
    frequency: row[3], oilType, brand: row[5], quantity: row[6],
    changeDate: formatDate(row[9]), nextDueDate: formatDate(row[10]),
...
export function oilChangeToRow(o) {
  return [
    o.equipmentCode || o.assetName || "", "", o.lubricationPoint || "",
    o.frequency || "Oil Analysis", o.oilType || "", o.brand || "",
    o.quantity || "", "", "", o.changeDate || "", o.nextDueDate || "",
    o.status || "Current",
  ];
}
...
export function rowToSample(row) {
  const [unitId, description, sampleId, sampledDate, reportStatus,
    contaminationRating, equipmentRating, lubricantRating, particleCount4um,
    particleCount6um, particleCount14um, pqIndex, visc40C, tan, oxidation,
    water, Ag, Al, Cr, Cu, Fe, Mo, Ni, Pb, Sn, K, Na, Si, B, Ba, Ca, Mg, P, Zn,
    , recommendationsRaw] = row;
  const num = (v) => (v === "" || v === null || v === undefined ? "" : parseFloat(v));
  return {
    unitId, description, sampleId, sampledDate: formatDate(sampledDate),
    reportedDate: formatDate(row[86]), reportStatus, contaminationRating,
    equipmentRating, lubricantRating,
    particleCount4um: num(particleCount4um), particleCount6um: num(particleCount6um),
    particleCount14um: num(particleCount14um), pqIndex: num(pqIndex),
    visc40C: num(visc40C), tan: num(tan), oxidation: num(oxidation), water: num(water),
...
export function sampleToRow(s) {
  ...
  return [
    ...
    s.reportStatus || "", s.contaminationRating || "", s.equipmentRating || "",
    s.lubricantRating || "", s.particleCount4um || "", s.particleCount6um || "",
    s.particleCount14um || "", s.pqIndex || "", s.visc40C || "", s.tan || "",
    s.oxidation || "", s.water || "", wear.Ag || "", wear.Al || "", wear.Cr || "",
    wear.Cu || "", wear.Fe || "", wear.Mo || "", wear.Ni || "", wear.Pb || "",
    wear.Sn || "", contaminants.K || "", contaminants.Na || "", contaminants.Si || "",
    additives.B || "", additives.Ba || "", additives.Ca || "", additives.Mg || "",
    additives.P || "", additives.Zn || "",
