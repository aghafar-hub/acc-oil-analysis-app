...
export default function Dashboard({ samples, actions, oilChanges, onSelectSample }) {
  const stats = useMemo(() => {
    ...
    const pendingActions = actions.filter(
      (a) => a.status === "Open" || a.status === "In Progress" || a.status === "Waiting Stoppage"
    ).length;
    return {
      critical,
      warning,
  ...
  const recent = useMemo(() => [...samples].sort((a, b) => new Date(b.sampledDate) - new Date(a.sampledDate)).slice(0, 10), [samples]);
      ...
      <div style={s.card}>
        ...
        {recent.length === 0 ? (
          ...
          <table style={s.table}>
            <thead>
              <tr>
                {["Equipment", "Sampled", "Status", ""].map((h) => (
                  <th key={h} style={s.th}>
                    {h}
                  </th>
                ))}
            ...
            <tbody>
              {recent.map((sam, i) => (
                <tr key={sam._id || i}>
                  ...
                  <td style={s.td}>
                    <span style={s.badge(sam.reportStatus)}>{sam.reportStatus}</span>
                  </td>
                  <td style={s.td}>
                    <button style={{ ...s.btn, padding: "3px 10px", fontSize: 12 }} onClick={() => onSelectSample(sam)}>
                      Open
                    </button>
                  </td>
                </tr>
