...
export default function AddSample({ equipmentOptions, equipmentRegistry, existingSamples, onAdd }) {
  const { T, s } = useTheme();
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 12 }}>
          {["reportStatus", "contaminationRating", "equipmentRating", "lubricantRating"].map((key) => (
            <div key={key}>
              ...
              <select style={{ ...s.input, fontSize: 12, cursor: "pointer" }} value={form[key]} onChange={(e) => set(key, e.target.value)}>
